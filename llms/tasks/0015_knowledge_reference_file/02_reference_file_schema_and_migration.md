...
- Add a reference-file-specific metadata table, likely `knowledge_reference_files`.
- Include fields for `knowledge_item_id`, stable `reference_ref`, flexible `reference_file_type`, original filename, content type, size, checksum, storage ref, metadata, and descriptor safety flags where appropriate.
- Add indexes for scope/status listing and metadata-first lookup by type/tags/status.
...
- Do not add DB `CHECK` constraints for reference file type, status, content type, safety flags, or lifecycle rules.

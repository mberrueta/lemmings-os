defmodule LemmingsOs.Config.RuntimeKnowledgeReferenceFileStorageConfigTest do
  use ExUnit.Case, async: false

  @runtime_config_path Path.expand("../../../config/runtime.exs", __DIR__)

  describe "knowledge reference-file storage runtime env contract" do
    test "uses LEMMINGS_KNOWLEDGE_REFERENCE_FILE_STORAGE_ROOT when set" do
      with_env(
        %{
          "LEMMINGS_KNOWLEDGE_REFERENCE_FILE_STORAGE_ROOT" => "/tmp/knowledge-reference-root"
        },
        fn ->
          storage = runtime_storage_config()
          assert Keyword.get(storage, :root_path) == "/tmp/knowledge-reference-root"
        end
      )
    end
  ...
  defp runtime_storage_config do
    ...
    Keyword.get(lemmings_os_config, :knowledge_reference_file_storage, [])
  end
